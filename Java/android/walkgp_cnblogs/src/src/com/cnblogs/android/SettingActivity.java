...
public class SettingActivity extends PreferenceActivity implements
	...
	private static final int DIALOG_FONT_SIZE = 2;// 字体设置对话框

	private static final TextSize fontSize = TextSize.SMALLEST;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		preferencescreen.findPreference(CONFIG_FONT_SIZE_OPTION_KEY)
				.setOnPreferenceChangeListener(this);
		// 清空缓存
		...
		listFontSize = (ListPreference) findPreference(CONFIG_FONT_SIZE_OPTION_KEY);

		String readMode = GetConfigReadMode(getApplicationContext());
		listReadMode.setSummary("当前选择：" + GetReadMode(readMode));
		listFontSize.setSummary("当前选择："
				+ GetConfigFontSize(getApplicationContext()).toString());
	...
	public static TextSize GetConfigFontSize(Context context) {
		return Enum.valueOf(
				TextSize.class,
				GetDefaultSharedPreferences(context).getString(
						CONFIG_FONT_SIZE_OPTION_KEY, "SMALLEST"));
	}
	...
	public boolean onPreferenceClick(Preference preference) {
		...
		if (key.equals(CONFIG_READ_MODE_OPTION_KEY)) {// 阅读模式
			...
			return true;
		} else if (key.equals(CONFIG_FONT_SIZE_OPTION_KEY)) {
	...
	public boolean onPreferenceChange(Preference preference, Object newValue) {
		if (preference == listReadMode) {
			String readMode = newValue.toString().equalsIgnoreCase("0") ? "图文模式"
					: "文字模式";
			...
			editor.commit();
		} else if (preference == listFontSize) {
			TextSize fontSize = Enum.valueOf(TextSize.class,
					newValue.toString());
			listFontSize.setSummary("当前选择：" + newValue.toString());
			SharedPreferences.Editor editor = sharePreferences.edit();
			editor.putString(CONFIG_FONT_SIZE_OPTION_KEY, newValue.toString());
			editor.commit();
		}
	...
	public void onClick(DialogInterface dialog, int which, boolean isChecked) {
		...
		if (dialog == dialogSelectReadMode) {
			...
			editor.commit();
		} else if (dialog == dialogFontSize) {
			SharedPreferences.Editor editor = sharePreferences.edit();
			editor.putInt(CONFIG_FONT_SIZE_OPTION_KEY, which);
			editor.commit();
		}
	...
	protected Dialog onCreateDialog(int dialogGuid) {
		...
		case DIALOG_FONT_SIZE:// 字体大小
			...
			dialogFontSize = (AlertDialog) mDialog;
			break;
		case DIALOG_CLEAR_CACHE:// 清空缓存
			// progressDialog = new ProgressDialog(SettingActivity.this);
			// progressDialog=ProgressDialog.show(SettingActivity.this, "获取信息",
			// "计算中，请稍候……",false,true);

			/*
			 * progressDialog = new ProgressDialog(SettingActivity.this);
			 * progressDialog.setTitle("获取信息");
			 * progressDialog.setMessage("计算中，请稍候……"); progressDialog.show();
			 */
